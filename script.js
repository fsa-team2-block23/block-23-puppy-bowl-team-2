...
const addNewPlayer = async (playerObj) => {
  try {
    const response = await fetch(`${API_URL}/${playerObj}`, {
      method: "POST",
      headers: {
...
const removePlayer = async (playerId) => {
  try {
    ...
    const rPlayer = await response.json();
    fetchAllPlayers();
    window.location.reload();
  } catch (err) {
    console.error(
...
const renderSinglePlayer = async (playerId) => {
  try {
    ...
    const closeButton = playerDetailsElement.querySelect(".close-button");
    closeButton.addEventListener("click", () => {
      playerDetailsElement.remove();
    });
  } catch (err) {
    console.error("Uh oh, trouble closing window!", err);
  }
};
